...
public class Main {
    ...
    public static int[] heroesHealth = {180, 270, 180, 150, 300, 270, 260, 170};
    public static int[] heroesDamage = {10, 15, 20, 0, 7, 15, 16, 16};
    static String[] heroesAttackType = new String[]{"Physical", "Magical", "Kinetic", "Medic", "Golem", "Lucky", "Berserk", "Thor"};
            ...
            public static void berserkAbility(){
                for (int i = 0; i < heroesAttackType.length; i++) {
                    if(heroesAttackType[i].equals("Berserk")){
                        int berserkDamage = heroesDamage[6] + bossDamage;
                        System.out.println("Герой " + heroesAttackType[6] + " использовал способность и нанес: " + berserkDamage + " боссу");
                    }
                }
            }
            ...
            public static void thorAbility() {
                for (int i = 0; i < heroesAttackType.length; i++) {
                    if (heroesAttackType[i].equals("Thor")) {
                        Random random = new Random();
                        boolean ThorAbilityVariable = random.nextBoolean();
                        if (heroesAttackType[i].equals("Thor") == ThorAbilityVariable) {
                            bossDamage = 0;
